...
document.addEventListener('DOMContentLoaded', () => {
    ...
    <div id="redirect-modal" class="fixed inset-0 bg-slate-900/50 backdrop-blur-sm z-[100] hidden items-center justify-center opacity-0 transition-opacity duration-300">
        <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl p-6 md:p-8 w-[90%] max-w-sm m-auto transform scale-95 transition-transform duration-300 text-center border border-slate-200 dark:border-slate-700 flex flex-col">
            ...
            <p class="text-slate-600 dark:text-slate-400 text-sm mb-6 flex-1">Você está sendo redirecionado para um ambiente externo (<span id="redirect-dest">WhatsApp</span>). Deseja continuar?</p>
            <div class="flex gap-3 justify-center w-full mt-auto">
                <button id="cancel-redirect" class="px-5 py-2.5 rounded-xl font-semibold text-sm text-slate-700 dark:text-slate-300 bg-slate-100 dark:bg-slate-700 hover:bg-slate-200 dark:hover:bg-slate-600 transition-colors flex-1">Cancelar</button>
                <button id="confirm-redirect" class="px-5 py-2.5 rounded-xl font-semibold text-sm text-white bg-blue-600 hover:bg-blue-500 transition-colors shadow-lg shadow-blue-600/20 flex-1">Continuar</button>
            </div>
        </div>
    </div>`;
    ...
    document.addEventListener('click', (e) => {
        ...
        // Do not intercept if click is inside the modal
        if (link.closest('#redirect-modal')) return;
